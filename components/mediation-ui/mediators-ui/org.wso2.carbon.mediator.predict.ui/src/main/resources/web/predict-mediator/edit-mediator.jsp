...
<carbon:jsi18n
            resourceBundle="org.wso2.carbon.mediator.predict.ui.i18n.JSResources"
            request="<%=request%>"
            i18nObjectName="mli18n"/>
<div>
<script type="text/javascript" src="../predict-mediator/js/mediator-util.js"></script>
<script type="text/javascript">
    var val;
    jQuery('#actionID').click(function() {
        val = document.getElementById('mediatorInputId').value;
        if(mlMediatorValidate()) {
            var url = '../predict-mediator/predict-ajaxprocessor.jsp';
            jQuery('#attribDescription').load(url, {mediatorInput: val, clearAll : 'true'},
                    function(res, status, t) {
                        if (status != "success") {
                            CARBON.showErrorDialog('<fmt:message key="mediator.predict.error.msg"/>');
                        }
                    })
            return false;
        }
...
<tr>
    <td>
        <h2><fmt:message key="mediator.predict.header"/></h2>
    </td>
</tr>
<tr>
    <td>
        <table class="normal">
            <tr>
                <td><fmt:message key="mediator.predict.model.name"/><span class="required">*</span>
                </td>
                <td align="left">
                ...
                <td>
                    <input id="actionID" type="button" value="<fmt:message key="mediator.predict.load.model"/>"
                           class="button"/>
                </td>
            </tr>
        </table>
...
<div class="Mediator" id="attribDescription">
    <% if (modelName != null && modelName.length() > 0) { %>
        <jsp:include page="predict-ajaxprocessor.jsp">
