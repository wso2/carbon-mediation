...
function iterateMediatorValidate() {
    ...
    if (expression && expression.value == "") {
        CARBON.showErrorDialog(iteratei18n["mediator.iterate.msg.specifyexpression"]);
        return false;
    }

    var preservePayload = getSelectedValue("preservePayload");
    var attachPath = document.getElementById("attach_path");

    if (attachPath && attachPath.value != "" && preservePayload == 'false') {
        CARBON.showErrorDialog(iteratei18n["mediator.iterate.msg.setpreservepayload"]);
        return false;
    }
