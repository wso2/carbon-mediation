...
@Component(
        name = "org.wso2.carbon.message.processor",
        immediate = true)
public class MessageProcessorServiceComponent extends AbstractAxis2ConfigurationContextObserver {
    ...
    protected void activate(ComponentContext ctxt) {
        ...
        bndCtx.registerService(MessageProcessorDeployerService.class.getName(), new
                MessageProcessorDeployerServiceImpl(), null);
        try {
    ...
    private void registerDeployer(AxisConfiguration axisConfig, SynapseEnvironment synapseEnvironment) {
        ...
        String messageProcessorDirPath = synapseConfigPath + File.separator + MultiXMLConfigurationBuilder
                .MESSAGE_PROCESSOR_DIR;
        for (MessageProcessor processor : synCfg.getMessageProcessors().values()) {
            if (processor.getFileName() != null) {
                deploymentStore.addRestoredArtifact(messageProcessorDirPath + File.separator + processor.getFileName());
            }
        }
        synchronized (axisConfig) {
            deploymentEngine.addDeployer(new MessageProcessorDeployer(), messageProcessorDirPath, ServiceBusConstants
                    .ARTIFACT_EXTENSION);
        }
    }
    ...
    private void unregisterDeployer(AxisConfiguration axisConfig, SynapseEnvironment synapseEnvironment) {
        ...
        if (axisConfig != null) {
            ...
            String synapseConfigPath = ServiceBusUtils.getSynapseConfigAbsPath(synapseEnvironment
                    .getServerContextInformation());
            String processorDirPath = synapseConfigPath + File.separator + MultiXMLConfigurationBuilder
                    .MESSAGE_PROCESSOR_DIR;
            deploymentEngine.removeDeployer(processorDirPath, ServiceBusConstants.ARTIFACT_EXTENSION);
        }
    }
