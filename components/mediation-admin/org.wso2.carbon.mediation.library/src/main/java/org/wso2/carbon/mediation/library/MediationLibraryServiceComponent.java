...
@Component(
        name = "org.wso2.carbon.mediation.library.service",
        immediate = true)
public class MediationLibraryServiceComponent extends AbstractAxis2ConfigurationContextObserver {
    ...
    protected void activate(ComponentContext ctxt) {
        ...
        try {
            ...
            bndCtx.registerService(LibraryDeployerService.class.getName(), new LibraryDeployerServiceImpl(), null);
    ...
    private void registerDeployer(AxisConfiguration axisConfig, SynapseEnvironment synapseEnvironment) {
        ...
        String synapseImportDir = synapseConfigPath + File.separator + MultiXMLConfigurationBuilder.SYNAPSE_IMPORTS_DIR;
        for (SynapseImport synImport : synCfg.getSynapseImports().values()) {
            if (synImport.getFileName() != null) {
                deploymentStore.addRestoredArtifact(synapseImportDir + File.separator + synImport.getFileName());
            }
        }
        // register imports
        synchronized (axisConfig) {
            deploymentEngine.addDeployer(new ImportDeployer(), synapseImportDir, ServiceBusConstants
                    .ARTIFACT_EXTENSION);
        }
    ...
    private void unregisterDeployer(AxisConfiguration axisConfig, SynapseEnvironment synapseEnvironment) {
        ...
        if (axisConfig != null) {
            ...
            String synapseConfigPath = ServiceBusUtils.getSynapseConfigAbsPath(synapseEnvironment
                    .getServerContextInformation());
            String importDirPath = synapseConfigPath + File.separator + MultiXMLConfigurationBuilder
                    .SYNAPSE_IMPORTS_DIR;
            deploymentEngine.removeDeployer(importDirPath, ServiceBusConstants.ARTIFACT_EXTENSION);
            String carbonRepoPath = axisConfig.getRepository().getPath();
            String libsPath = carbonRepoPath + File.separator + "synapse-libs";
