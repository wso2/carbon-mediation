 ...
 * proxy services.
 */
@SuppressWarnings({ "UnusedDeclaration" }) public class ProxyServiceAdmin extends AbstractServiceBusAdmin {
        ...
        try {
            ...
            if (proxy != null) {
                ...
                if (!cAppArtifactDataService
                        .isArtifactDeployedFromCApp(getTenantId(), getArtifactName(artifactType, proxyName))) {
                    persistProxyService(proxy);
                }
            } else {
                log.error("Couldn't find the proxy service with name " + proxyName + " to enable statistics");
                return FAILED;
            }
    ...
    public String disableStatistics(String proxyName) throws ProxyAdminException {
        ...
        try {
            ...
            if (proxy != null) {
                ...
                log.error("Couldn't find the proxy service with name " + proxyName + " to disable statistics");
                return FAILED;
            }
            if (log.isDebugEnabled()) {
     ...
     *                             same name exists
     */
    private void addProxyService(OMElement proxyServiceElement, String fileName, boolean updateMode)
        ...
        try {
            if (proxyServiceElement.getQName().getLocalPart().equals(XMLConfigConstants.PROXY_ELT.getLocalPart())) {
                ...
                if (getSynapseConfiguration().getProxyService(proxyName) != null
                        || getSynapseConfiguration().getAxisConfiguration().getService(proxyName) != null) {
                    handleException(log, "A service named " + proxyName + " already exists", null);
                } else {
                    ProxyService proxy = ProxyServiceFactory
                            .createProxy(proxyServiceElement, getSynapseConfiguration().getProperties());
            ...
            if (proxyServiceElement.getQName().getLocalPart().equals(XMLConfigConstants.PROXY_ELT.getLocalPart())) {
                ...
                if (currentProxy == null) {
                    ...
                    if (axisService != null) {
                        ...
                        axisService.getParent().addParameter(CarbonConstants.KEEP_SERVICE_HISTORY_PARAM, "true");
                    ...
                    try {
                        ...
                        log.error("Unable to save changes made for the proxy service : " + proxyName
                                + ". Restoring the existing proxy..");
                        ...
                        handleException(log, "Unable to save changes made for the proxy service : " + proxyName
                                + ". Restored the existing proxy...", e);
                    }
                }
            } else {
                handleException(log, "Invalid proxy service definition", null);
    ...
    public String startProxyService(String proxyName) throws ProxyAdminException {
        ...
        try {
            ...
            if (pinnedServers.isEmpty() || pinnedServers
                    .contains(getServerConfigurationInformation().getServerName())) {
    ...
    public String stopProxyService(String proxyName) throws ProxyAdminException {
        ...
        try {
            ...
            if (pinnedServers.isEmpty() || pinnedServers
                    .contains(getServerConfigurationInformation().getSynapseXMLLocation())) {
    ...
    private ProxyData generateProxyDataFor(ProxyService ps) throws ProxyAdminException {
        ...
        if (ps.getTargetEndpoint() != null) {
            ...
            pd.setEndpointXML(EndpointSerializer.getElementFromEndpoint(ps.getTargetInLineEndpoint()).toString());
        }
        ...
        if (pd.isWsdlAvailable()) {
            ...
            if (ps.getResourceMap() != null && (map = ps.getResourceMap().getResources()) != null && !map.isEmpty()) {
     ...
     * @throws AxisFault
     */
    private void updateAndSyncServiceParameters(ProxyService service, AxisService axisService)
            throws ProxyAdminException, AxisFault {
            ...
            if (registry.resourceExists(serviceParametersPath)) {
                ...
                if (serviceParamsResource instanceof Collection) {
                    ...
                    for (String serviceParamResourcePath : serviceParamsCollection.getChildren()) {
                        ...
                        String[] elems = serviceParamResourcePath.split(RegistryConstants.PATH_SEPARATOR);
