...
public class QueryPayloadFactory extends PayloadFactory {
    @Override
    public OMElement createPayload(Object... params) {
        String queryStr = (String) params[0];
        OMFactory factory = OMAbstractFactory.getOMFactory();
        OMNamespace ns = factory.createOMNamespace("http://wso2.services.sample",
                                                   "ns");

        OMElement queryReq = factory.createOMElement("query",
                                                     ns);

        OMElement queryStrElem = factory.createOMElement("queryString", ns);

        queryStrElem.setText(queryStr);

        queryReq.addChild(queryStrElem);
        return queryReq;
