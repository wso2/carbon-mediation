...
public abstract class AbstractStore<T> {
    ...
    public void save(String name, SynapseConfiguration config) {
        ...
        if (fileName == null) {
            ...
            element = saveToFile(obj, config);
        }

        // Save the element to the registry
    ...
    public void saveItemToRegistry(String name, SynapseConfiguration config) {
        ...
        OMElement element = serialize(obj);
        saveToRegistry(name, element, getFileName(obj));
    }
    ...
    protected void saveToRegistry(String name, OMElement element, String fileName) {
        if (element != null && registryStore != null) {
            if (name == null) {
                log.warn("Name of the configuration item to be saved is not given");
                return;
            }
            registryStore.persistElement(name, element, fileName);
        }
    }
    ...
    protected void deleteFromRegistry(String name) {
        if (registryStore != null) {
            if (name == null) {
                log.warn("Name of the configuration item to be deleted is not given");
                return;
            }
            registryStore.deleteElement(name);
        }
    }
