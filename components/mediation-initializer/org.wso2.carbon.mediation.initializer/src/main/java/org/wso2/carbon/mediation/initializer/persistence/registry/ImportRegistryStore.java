...
public class ImportRegistryStore extends AbstractRegistryStore {
    public ImportRegistryStore(UserRegistry registry, String configName) {
        super(registry, configName);
        createCollection(getConfigurationPath());
    }

    public Collection<OMElement> getElements() {
        Collection<OMElement> importElements = new ArrayList<OMElement>();
        try {
            importElements = getChildElementsInPath(getConfigurationPath());
    ...
    public void persistElement(String name, OMElement element, String fileName) {
        if (element.getLocalName().equals(
                XMLConfigConstants.IMPORT_ELT.getLocalPart())) {
            if (log.isDebugEnabled()) {
                log.debug("Persisting API: " + name + " to the registry");
            }

            String importPath = getConfigurationPath() + RegistryConstants.PATH_SEPARATOR + name;

            try {
                persistElement(element, importPath, fileName);
            } catch (RegistryException e) {
                handleException("Unable to persist the API in the path : " + importPath, e);
            }
        } else {
    ...
    protected String getConfigurationPath() {
        return getConfigurationRoot() + RegistryConstants.PATH_SEPARATOR +
                ServiceBusConstants.RegistryStore.IMPORT_REGISTRY;
    }
}
