...
public class InboundRequestProcessorFactoryImpl implements InboundRequestProcessorFactory {
    ...
    /**
     * 
     * Returns the related InboundRequestProcessor based on protocol/classimpl
     * 
     * @param InboundProcessorParams
    ...
    public InboundRequestProcessor createInboundProcessor(InboundProcessorParams params) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Creating the Inbound Processor for protocol " + protocol + " or class "
                    + params.getClassImpl());
        }
        if (protocol != null) {
            ...
            log.error("Invalid inbound endpoint synapse configuration. Protocol or Class should be specified.");
        }
        return inboundRequestProcessor;
    }
