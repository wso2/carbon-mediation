...
public abstract class InboundRequestProcessorImpl implements InboundRequestProcessor {
    ...
    protected void start(InboundTask task, String endpointPostfix) {
        log.info("Starting the inbound endpoint " + name + ", with coordination " + coordination + endpointPostfix);
        if (coordination) {
            try {
                ...
                if(cron != null){
                    taskDescription.setCronExpression(cron);
                    log.info("Task cron expression : " + cron);
                } else{
                    ...
                    log.info("Task interval : " + interval);
                }
