...
import org.wso2.carbon.inbound.endpoint.protocol.http.core.impl.InboundSourceContext;
...
public class InboundSourceConnections {
    ...
    public void releaseConnection(NHttpServerConnection conn) {
        lock.lock();
        try {
            InboundSourceContext.get(conn).reset();
    ...
    public void shutDownConnection(NHttpServerConnection conn, boolean isError) {
        ...
        lock.lock();
        try {
            InboundSourceContext.get(conn).reset(isError);
