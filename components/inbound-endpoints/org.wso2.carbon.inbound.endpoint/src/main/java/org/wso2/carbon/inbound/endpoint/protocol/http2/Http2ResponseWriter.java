...
public class Http2ResponseWriter {
    ...
    public void writeNormalResponse(MessageContext synCtx) {
        ...
        if (noEntityBody == null || Boolean.FALSE == noEntityBody) {
            ...
            if (msgContext.getProperty(org.apache.axis2.Constants.Configuration.MESSAGE_TYPE)
                    == null) {
                transportHeaders.add(HTTP.CONTENT_TYPE, messageFormatter
        ...
        if (hasBody) {
            ...
            Pipe pipe = msgContext.getProperty("pass-through.pipe") == null ?
                    null :
    ...
    public void writePushPromiseResponse(MessageContext synCtx) {
        ...
        LinkedList<Http2StreamFrame> pushFrames = (LinkedList<Http2StreamFrame>) ((Axis2MessageContext) synCtx)
                .getAxis2MessageContext()
                .getProperty(InboundHttp2Constants.HTTP2_PUSH_PROMISE_DATA);
        ...
        while (iterator.hasNext()) {
            ...
            if (frame instanceof Http2HeadersFrame) {
                if (promise_id == 0)
                    return;
                ChannelPromise promise = c.newPromise();
                encoder.writeHeaders(c, promise_id, ((Http2HeadersFrame) frame).headers(), 0,
                        ((Http2HeadersFrame) frame).isEndStream(), promise);
            } else if (frame instanceof Http2DataFrame) {
                if (promise_id == 0)
                    return;
                ChannelPromise promise = c.newPromise();
                byte [] data=ByteBufUtil.getBytes(((Http2DataFrame)frame).content());
                encoder.writeData(c, promise_id, Unpooled.copiedBuffer(data), 0,
                        ((Http2DataFrame) frame).isEndStream(), promise);

                ((Http2DataFrame)frame).content().release();
            } else {
                if (!promise_ids.containsKey(id)) {
