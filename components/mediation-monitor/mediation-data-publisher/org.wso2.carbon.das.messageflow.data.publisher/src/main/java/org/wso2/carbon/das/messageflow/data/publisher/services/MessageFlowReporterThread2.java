        ...
        while (!shutdownRequested) {
            try {
                ...
                if (statisticsReportingEventHolder != null) {
                    MessageFlowProcessorInterface flowProcessor = new MessageFlowProcessor(messageFlowObserverStore);
                    log.info("******* event list size - " + statisticsReportingEventHolder.getEventList().size() + " holder count - " + statisticsReportingEventHolder.countHolder.getStatCount() + " callback - " + statisticsReportingEventHolder.countHolder.getCallBackCount());
                    for (StatisticsReportingEvent event : statisticsReportingEventHolder.getEventList()) {
                        event.processEvents(flowProcessor);
                    }
                } else {
                    Thread.sleep(1);
                }
